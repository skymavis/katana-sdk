...
export class V2HeuristicGasModelFactory extends IV2GasModelFactory {
  public async buildGasModel({
    ...
    const usdPoolPromise: Promise<Pair> = this.getHighestLiquidityUSDPool(chainId, poolProvider, providerConfig);
    ...
    const nativeAndSpecifiedGasTokenPoolPromise =
      providerConfig?.gasToken && !providerConfig?.gasToken.equals(WRAPPED_NATIVE_CURRENCY[chainId])
    ...
    const calculateL1GasFees = async (): Promise<{
      ...
      return await calculateL1GasFeesHelper(chainId, usdPool, token, nativePool);
    };
