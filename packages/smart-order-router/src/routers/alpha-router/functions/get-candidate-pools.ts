...
export async function getV3CandidatePools({
  ...
  if (WRAPPED_NATIVE_CURRENCY[chainId]?.symbol == 'WRON' && tokenOut.symbol != 'RON' && tokenOut.symbol != 'WRON') {
...
export async function getMixedRouteCandidatePools({
  ...
  const V3tokenPairsRaw = _.map<V3SubgraphPool, [Token, Token, FeeAmount] | undefined>(V3sortedPools, subgraphPool => {
    const tokenA = tokenAccessor.getTokenByAddress(subgraphPool.token0.id);
    const tokenB = tokenAccessor.getTokenByAddress(subgraphPool.token1.id);
    let fee: FeeAmount;
    try {
