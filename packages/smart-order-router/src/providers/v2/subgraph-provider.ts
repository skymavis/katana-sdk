...
const SUBGRAPH_URL_BY_CHAIN: { [chainId in ChainId]?: string } = {
  [ChainId.mainnet]: 'https://thegraph-v2.roninchain.com/subgraphs/name/axieinfinity/katana-subgraph-blue',
  ...
  public async getPools(
    ...
    await retry(
      ...
      },
      {
        ...
        onRetry: (err, retry) => {
          outerRetries += 1;
          if (this.rollback && blockNumber && _.includes((err as any).message, 'indexed up to')) {
    ...
    const tracked = pools.filter(
      pool =>
        pool.token0.id == FEI || pool.token1.id == FEI || parseFloat(pool.trackedReserveETH) > this.trackedEthThreshold,
    );
